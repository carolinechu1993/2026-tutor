...
4. **具體**：提到實際單元或題型名稱（例：「魔數大戰」「量角器」「7-4 角與角度」「尾數是 0 的除法」「被除數／除數」），描述他實際發生什麼事、老師怎麼處理、結果如何。

5. **兼顧正向與誠實**：好的地方要講，問題也要直白指出，可以帶入情緒觀察（不耐煩、自信、分心、洋洋灑灑地寫、粗心大意）。

6. **不要制式評語堆砌**（「很棒很用心」「表現優異」這種空話一律不要）。

7. **格式**：一段連貫文字，**不要標題、不要條列、不要引號**。
...
**語氣要求（重要）**：Q8、Q9 要寫得像**你在跟下學期接手的老師面對面交接**，是同事之間的口語對談，不是寫公文。
